...
impl FromStr for r64 {
    ...
    fn from_str(mut src: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(pos) = src.find('/') {
            ...
            let numerator: u64 = src[0..pos].parse().unwrap();
            let denominator: u64 = src[pos+1..].parse().unwrap();
            ...
            if numerator == 1 && denom_size == 26 {
                ...
                Ok(r64::from_parts(s, 1, denominator))
            }
            else {
                ...
                if frac_size as u64 > FRACTION_SIZE {
                    ...
                    Ok(r64::from_parts(s, numerator, denominator))
                }
            }
        }
        else {
            let numerator: u64 = src.parse().unwrap();
            ...
            Ok(r64::from_parts(s, numerator, 1))
        }
    }
}
